<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-slate-800 leading-tight">Editar Equipamento</h2>
    </x-slot>
        ...
        <div class="bg-white border border-slate-200 rounded-xl shadow-sm p-6">
            <form method="POST" action="{{ route('equipamentos.update', $equipamento) }}" class="space-y-5">
                    ...
                    <div>
                        ...
                        <select name="user_id" class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500">
                            @foreach ($clientes as $cliente)
                                <option value="{{ $cliente->id }}" @selected(old('user_id', $equipamento->user_id) == $cliente->id)>{{ $cliente->name }}</option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <input type="text" name="tipo" value="{{ old('tipo', $equipamento->tipo) }}" class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="marca" value="{{ old('marca', $equipamento->marca) }}" class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500">
                    ...
                    <div>
                        ...
                        <input type="text" name="modelo" value="{{ old('modelo', $equipamento->modelo) }}" class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500">
                ...
                <div>
                    ...
                    <input type="text" name="numero_serie" value="{{ old('numero_serie', $equipamento->numero_serie) }}" class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500">
                    @error('numero_serie') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Descrição</label>
                    <textarea name="descricao" rows="3" class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500">{{ old('descricao', $equipamento->descricao) }}</textarea>
